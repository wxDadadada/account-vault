...
export function Accounts({
  ...
  const [view, setView] = useDisplayPreference(
    'account-view',
    ['grid', 'list'] as const,
    window.matchMedia('(max-width: 767px)').matches ? 'list' : 'grid'
  )
  ...
  return (
    <div className='collection-workbench'>
      ...
      <CollectionOverview
        ...
        favorites={data.accounts.filter((a) => a.favorite).length}
      ...
      <section className='collection-surface' aria-label='账号列表'>
        <div className='collection-commandbar'>
          ...
          >
            ...
            >
              待完善{' '}
              <span>{scope.filter((a) => a.status === 'pending').length}</span>
            </Button>
          </div>
              ...
              >
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setSelected((ids) =>
                      checked
                        ? [...ids.filter((id) => id !== account.id), account.id]
                ...
                <div className='entry-dates'>
                  <time
                    title={new Date(account.updatedAt).toLocaleString('zh-CN')}
                  >
      ...
      <div className='collection-footnote'>
        {demo ? '示例空间 · 本次体验不会保存' : '账号与历史在设备上加密后保存'}
